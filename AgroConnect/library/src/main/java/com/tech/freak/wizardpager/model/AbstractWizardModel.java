...
public abstract class AbstractWizardModel implements ModelCallbacks {
    ...
    public void addPage(Page page) {
        mRootPageList.add(page);
    }
    public void deleteLastPage() {
        mRootPageList.remove(mRootPageList.size()-1);
    }
    public void deletePagesAfter(int index) {
        int currentSize = mRootPageList.size()-1;
        for(int i = currentSize; i > index; i--)
            mRootPageList.remove(i);
    }
